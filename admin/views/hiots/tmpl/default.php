...
if ($saveOrder)
{
	$saveOrderingUrl = 'index.php?option=com_hiots&task=hiots.saveOrderAjax&tmpl=component';
	JHtml::_('sortablelist.sortable', 'hiotList', 'adminForm', strtolower($listDirn), $saveOrderingUrl);
}
?>
<form action="<?php echo JRoute::_('index.php?option=com_hiots&view=hiots'); ?>" method="post" name="adminForm" id="adminForm">
	...
	<?php endif;?>
		...
		<?php else : ?>
			<table class="table table-striped" id="hiotList">
				<thead>
					<tr>
						...
						<th width="10%" class="nowrap hidden-phone">
							<?php echo JHtml::_('searchtools.sort', 'Token', 'token', $listDirn, $listOrder); ?>
						</th>
				...
				<?php foreach ($this->items as $i => $item) : ?>
					...
					<tr class="row<?php echo $i % 2; ?>" sortable-group-id="<?php echo $item->catid; ?>">
						...
						<td class="center">
							<div class="btn-group">
								<?php echo JHtml::_('jgrid.published', $item->state, $i, 'hiots.', $canChange, 'cb', $item->publish_up, $item->publish_down); ?>
								<?php // Create dropdown items and render the dropdown list. ?>
								<?php if ($canChange) : ?>
									<?php JHtml::_('actionsdropdown.' . ((int) $item->state === 2 ? 'un' : '') . 'archive', 'cb' . $i, 'hiots'); ?>
									<?php JHtml::_('actionsdropdown.' . ((int) $item->state === -2 ? 'un' : '') . 'trash', 'cb' . $i, 'hiots'); ?>
						...
						<td class="nowrap has-context">
							<?php if ($item->checked_out) : ?>
								<?php echo JHtml::_('jgrid.checkedout', $i, $item->editor, $item->checked_out_time, 'hiots.', $canCheckin); ?>
							<?php endif; ?>
							<?php if ($canEdit || $canEditOwn) : ?>
								<a href="<?php echo JRoute::_('index.php?option=com_hiots&task=hiot.edit&id=' . (int) $item->id); ?>">
