...
/**
 * View class for a list of hiots.
 *
 * @since  1.5
 */
...
//		$this->filterForm    = $this->get('FilterForm');
	...
	protected function addToolbar()
	{
		...
		JToolbarHelper::title(JText::_('COM_HIOTS_MANAGER_HIOTS'), 'link hiots');

		if (count($user->getAuthorisedCategories('com_hiots', 'core.create')) > 0)
		{
			JToolbarHelper::addNew('hiot.add');
		}

		if ($canDo->get('core.edit') || $canDo->get('core.edit.own'))
		{
			JToolbarHelper::editList('hiot.edit');
		}

		if ($canDo->get('core.edit.state'))
		{
			JToolbarHelper::publish('hiots.publish', 'JTOOLBAR_PUBLISH', true);
			JToolbarHelper::unpublish('hiots.unpublish', 'JTOOLBAR_UNPUBLISH', true);

			JToolbarHelper::archiveList('hiots.archive');
			JToolbarHelper::checkin('hiots.checkin');
		}

		if ($state->get('filter.published') == -2 && $canDo->get('core.delete'))
		{
			JToolbarHelper::deleteList('JGLOBAL_CONFIRM_DELETE', 'hiots.delete', 'JTOOLBAR_EMPTY_TRASH');
		}
		elseif ($canDo->get('core.edit.state'))
		{
			JToolbarHelper::trash('hiots.trash');
		}
