...
<?php 	$user	= JFactory::getUser();
	...
	<?php if ($this->item->iot_type==0):?>
		...
		<div class="row-fluid form-horizontal"><?php echo JText::_('COM_HIOTS_ENDVERGAERUNGSGRAD');?>: <?php echo number_format( $this->endvergaerung->sevg, 0, JText::_('DECIMALS_SEPARATOR'), JText::_('THOUSANDS_SEPARATOR'))."/".
			number_format( $this->endvergaerung->tevg, 0, JText::_('DECIMALS_SEPARATOR'), JText::_('THOUSANDS_SEPARATOR'))."%";?></div> 	
		...
		<ul class="">
			<?php if ($this->vergaerung3->gaerdauer>2):?>		
				<li><?php echo JText::sprintf('COM_HIOTS_FERMENTATION_IN_LAST_DAYS',$this->vergaerung3->gaerdauer).number_format(($this->vergaerung3->stammwuerze-$this->vergaerung3->restextract), 1, JText::_('DECIMALS_SEPARATOR'), JText::_('THOUSANDS_SEPARATOR')).'° ('.
					number_format(($this->vergaerung3->stammwuerze-$this->vergaerung3->restextract)/$this->vergaerung3->gaerdauer, 2, JText::_('DECIMALS_SEPARATOR'), JText::_('THOUSANDS_SEPARATOR')).'°) Plato</li>';?></li>
			<?php endif ?>		
			<?php if ($this->vergaerung2->gaerdauer>1):?>		
				<li><?php echo JText::sprintf('COM_HIOTS_FERMENTATION_IN_LAST_DAYS',$this->vergaerung2->gaerdauer).number_format(($this->vergaerung2->stammwuerze-$this->vergaerung2->restextract), 1, JText::_('DECIMALS_SEPARATOR'), JText::_('THOUSANDS_SEPARATOR')).'° ('.
					number_format(($this->vergaerung2->stammwuerze-$this->vergaerung2->restextract)/$this->vergaerung2->gaerdauer, 2, JText::_('DECIMALS_SEPARATOR'), JText::_('THOUSANDS_SEPARATOR')).'°) Plato</li>';?></li>
			<?php endif ?>		
			<?php if ($this->vergaerung1->gaerdauer>0):?>		
				<li><?php echo JText::sprintf('COM_HIOTS_FERMENTATION_IN_LAST_DAYS',$this->vergaerung1->gaerdauer).number_format(($this->vergaerung1->stammwuerze-$this->vergaerung1->restextract), 1, JText::_('DECIMALS_SEPARATOR'), JText::_('THOUSANDS_SEPARATOR')).'° ('.
					number_format( ($this->vergaerung1->stammwuerze-$this->vergaerung1->restextract)/$this->vergaerung1->gaerdauer, 2, JText::_('DECIMALS_SEPARATOR'), JText::_('THOUSANDS_SEPARATOR')).'°) Plato</li>';?></li>
			<?php endif ?>		
		</ul>
		<div>Aktueller Alkoholgehalt: <?php echo number_format( $this->endvergaerung->alk, 1, JText::_('DECIMALS_SEPARATOR'), JText::_('THOUSANDS_SEPARATOR'))."Vol%";?></div> 	
	<?php else:?>
		<?php echo 'On: '.number_format(($this->stati->sumon/3600),1) .'h ('.number_format($this->stati->sumon*100/($this->stati->sumon+$this->stati->sumoff),0).'%)<br>';?>
		<?php echo 'Off:'.number_format(($this->stati->sumoff/3660),1) .'h ('.number_format($this->stati->sumoff*100/($this->stati->sumon+$this->stati->sumoff),0).'%)<br>';?>
	<?php endif;?>
